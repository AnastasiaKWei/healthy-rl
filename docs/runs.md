...
arms), which ran 2026-08-16 with per-token capture on and are complete.
...
- **Persona + psychoeducation + vignette.** In her render
  (`docs/prompts/v3.md`, which is affect-on throughout) turn 1 runs 369–414
  words for the four rewritten arms against a 110-word baseline; their v2
  selves were 237 (growth) and 278 (resilience), and appraisal, the block she
  kept, is unchanged (304 in her v2 render against 303 in her v3 one — the same
  text, a one-word drift in her boilerplate). Her rows pair with the `aff*`
  rows of `docs/prompts-rollouts.md`, which count every one of them exactly 34
  words lower; the same-named affect-off rows are 68 lower. Either gap is a
  fixed boilerplate difference between the two renders, not a difference in the
  blocks. The greppable `ruled out:` / `status check:` prefixes are gone
...
equal a fresh dry run. Every cell writes a fresh `out_dir` named for its `…6v3`
version, which is forced rather than tidy: `check_resume_mindset` refuses a v2
mindset directory under v3 code, so pointing one of these at its v2 namesake
would fail at startup. Three shards of eight, `--gres=gpu:A100-40G:2`, 4 h,
