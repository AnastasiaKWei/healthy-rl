#!/usr/bin/env bash
# Generate the inoculation-arm shard configs and submit their slurm DAG.
#
#   scripts/inoculation_cells.sh --dry-run    # write configs, print sbatch lines (default)
#   scripts/inoculation_cells.sh --submit     # write configs, submit
#
#   ONLY_MODEL=gemma-3-12b-it scripts/inoculation_cells.sh --submit   # just that model
#   ONLY_BASE=aff6r scripts/inoculation_cells.sh --submit             # just that base cell
#
# Cells (.superpowers/sdd/inoculation-arm.md, design approved 2026-08-16):
#   priority 1  gemma-3-12b-it                d6r-base    inoc6      nice 0
...
#
# The shard config is the model's base-cell config with the `inoculation:` key and
# its own out_dir. max_tokens is NOT touched -- the `r` base configs are already at
# 24576. Everything else is byte-identical, which is what keeps the cells comparable.
...
# model|version|base_version|nice
CELLS=(
  "gemma-3-12b-it|inoc6|d6r|0"
...
write_config() {  # model version base shard_index
  local model=$1 version=$2 base=$3 i=$4
  ...
  # out_dir -> the new cell; then append the inoculation block. max_tokens is left
  # alone: the `r` base configs are already at 24576.
  sed -e "s#^out_dir: .*#out_dir: /out/rollouts/${model}/${version}#" "$src" > "$tmp"
  cat >> "$tmp" <<EOF
...
selected=0
for row in "${CELLS[@]}"; do
  IFS='|' read -r model version base nice <<< "$row"
...
for row in "${CELLS[@]}"; do
  ...
  in_scope "$model" "$base" || continue
  for i in 0 1 2; do
    write_config "$model" "$version" "$base" "$i"
  done
done
